...
理解了上面的原理之后，设想如果我们将顺时针改为逆时针，则记录点的顺序应该就是 A -> D -> C -> B, 再使用setLastPoint则改变的是B的位置，我们试试看结果和我们的猜想是否一致：
...
> PS：由于是使用的MarkDown，文件中最占体积的就是文字，我一般会控制文件大小在25Kb以下，太大的话不论是写起来还是读起来都很耗费时间。
